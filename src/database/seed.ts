...
const seed = async (connection: Connection): Promise<void> => {
    ...
    if (totalEntityCount === 0) {
        ...
        // Create users
        const users = userSeedData.map(userData => {
        ...
        const courseSeedData = [
            ...
            },
            {
                ...
                description:
                    "In this course, you'll learn how to test programs.",
            },
        ];

        // Create courses
        const courses = courseSeedData.map(courseData => {
            ...
            course.estimatedTime = estimatedTime || null;
            course.materialsNeeded = materialsNeeded || null;
            course.creator = user;
